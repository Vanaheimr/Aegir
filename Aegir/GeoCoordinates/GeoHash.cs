...
namespace org.GraphDefined.Vanaheimr.Aegir
{
...

    {
        ...
        public T Decode<T>(Func<Latitude, Longitude, T> Processor, Byte Digits = 12)
        {
            ...
            Double[] LatitudeInterval  = { GeoCoordinate.MinLatitude.Value,  GeoCoordinate.MaxLatitude.Value  };
            Double[] LongitudeInterval = { GeoCoordinate.MinLongitude.Value, GeoCoordinate.MaxLongitude.Value };

            foreach (var Character in this.InternalGeoHash)
            {
            ...
            return Processor(
                new Latitude (Math.Round((LatitudeInterval[0]  + LatitudeInterval[1])  / 2, Digits)),
                new Longitude(Math.Round((LongitudeInterval[0] + LongitudeInterval[1]) / 2, Digits))
            );
